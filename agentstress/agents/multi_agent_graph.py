...
class MultiAgentGraphAgent(BaseAgent):
    ...
    def __init__(
        self,
        agent_id: str = "agent_5_multi_agent_graph",
        model: str = "gpt-4o",
    ...
    def _build_graph(self):
        ...
        def verification_node(state: GraphState):
            res = self.llm.invoke(
                f"Fact-check this answer against the task: {state['instruction']}\nAnswer: {state['draft']}\nIdentify errors."
            ).content
    ...
    def run(self, instruction: str, instruction_type: str) -> AgentResult:
        ...
        try:
            ...
            assess = self.llm.invoke(
                f"Rate confidence (0-10) for: {instruction}. Return number only."
            ).content
            try:
                conf = int(assess.strip())
            ...
            return AgentResult(
                ...
                total_steps=len(result["tool_calls"]) + 3,
                duration_seconds=duration,
                completed=True,
                run_id=run_id,
                confidence_self_assessment=conf,
                steps_completed=["Search", "Synthesis", "Verification", "Merger"],
            )
