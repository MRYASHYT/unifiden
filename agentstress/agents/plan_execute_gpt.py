...
class PlanExecuteGPTAgent(BaseAgent):
    ...
    def __init__(
        self,
        agent_id: str = "agent_2_plan_execute",
        model: str = "gpt-4o",
    ...
    def run(self, instruction: str, instruction_type: str) -> AgentResult:
        ...
        try:
            ...
            llm = ChatOpenAI(model=self.model, temperature=self.temperature)
            assess_res = llm.invoke(
                f"Rate confidence (0-10) for task: {instruction}. Return number only."
            ).content
            try:
                conf = int(assess_res.strip())
            ...
            return AgentResult(
                ...
                total_steps=len(handler.tool_calls),
                duration_seconds=duration,
                completed=True,
                run_id=run_id,
                confidence_self_assessment=conf,
                steps_completed=["Planned", f"Executed {len(handler.tool_calls)} steps"],
            )
