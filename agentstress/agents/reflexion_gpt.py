...
class ReflexionGPTAgent(BaseAgent):
    ...
    def __init__(
        self, agent_id: str = "agent_3_reflexion", model: str = "gpt-4o", temperature: float = 0.1
    ):
    ...
    def _execute_with_tools(self, instruction: str) -> Dict[str, Any]:
        ...
        if "SEARCH:" in content:
            ...
            draft_res = self.llm.invoke(
                draft_prompt.format(input=instruction, results=search_result)
            )
    ...
    def run(self, instruction: str, instruction_type: str) -> AgentResult:
        ...
        try:
            ...
            critique = self.llm.invoke(
                critique_prompt.format(input=instruction, draft=draft)
            ).content
            ...
            final_answer = self.llm.invoke(
                revision_prompt.format(input=instruction, draft=draft, critique=critique)
            ).content

            duration = time.time() - start_time

            # Self-assessment
            assess_res = self.llm.invoke(
            ...
            return AgentResult(
                ...
                total_steps=3 + len(execution["tool_calls"]),
                duration_seconds=duration,
                completed=True,
                run_id=run_id,
                confidence_self_assessment=confidence,
                steps_completed=["Execute", "Critique", "Revise"],
            )
    ...
    def run_with_peer_context(
        ...
        return DebateHelper.run_debate_round(
            self.llm, self.agent_id, instruction, round_number, peer_data
        )
