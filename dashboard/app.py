            ...
            data.append(
                {
                    ...
                    "FAILURE_MODE": (
                        entry["data"].get("judgment", {}).get("failure_mode")
                        if "judgment" in entry["data"]
                        else entry["data"].get("failure_classification", {}).get("failure_mode")
                    ),
...
with st.sidebar:
    ...
    if st.button("INITIATE_STRESS_TEST", use_container_width=True):
        with st.spinner("EXECUTING_PROTOCOL..."):
            ...
            subprocess.run(
                [sys.executable, "agentstress/experiments/native_pilot.py"],
...
if df.empty:
    ...
    with m3:
        if is_secure:
            st.markdown(
                '<div class="status-card secure-card">🛡️ SYSTEM_LEDGER: AUTHENTIC</div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                '<div class="status-card danger-card">⚠️ SYSTEM_LEDGER: COMPROMISED</div>',
                unsafe_allow_html=True,
            )
